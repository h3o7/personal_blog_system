    ...
    <div class="content-section">
      <el-tabs v-model="activeTab" @tab-click="handleTabClick" class="content-tabs">
        <el-tab-pane name="liked">
          ...
          <div 
            class="article-list" 
            v-loading="likedLoading && likedBlogList.length === 0"
            ref="likedContainerRef"
          >
            <el-card
              v-for="article in likedBlogList"
            ...
            <!-- 加载更多指示器 -->
            <div v-if="likedLoadingMore" class="loading-more">
              <el-icon class="is-loading"><Loading /></el-icon>
              <span>加载更多文章...</span>
            </div>
            
            <!-- 没有更多数据提示 -->
            <div v-if="likedNoMoreData && likedBlogList.length > 0" class="no-more-data">
              <el-divider>已经到底了~</el-divider>
            </div>
            
            <!-- 空状态 -->
            <div v-if="likedBlogList.length === 0 && !likedLoading" class="empty-state">
              <el-empty description="还没有喜欢的文章" />
            </div>
          </div>
        </el-tab-pane>
        ...
        <el-tab-pane name="collected">
          ...
          <div 
            class="article-list" 
            v-loading="collectedLoading && collectedBlogList.length === 0"
            ref="collectedContainerRef"
          >
            <el-card
              v-for="article in collectedBlogList"
              :key="`${article.userName}-${article.articleAddTime}`"
              class="article-card"
              shadow="hover"
            >
            ...
            <!-- 加载更多指示器 -->
            <div v-if="collectedLoadingMore" class="loading-more">
              <el-icon class="is-loading"><Loading /></el-icon>
              <span>加载更多文章...</span>
            </div>
            
            <!-- 没有更多数据提示 -->
            <div v-if="collectedNoMoreData && collectedBlogList.length > 0" class="no-more-data">
              <el-divider>已经到底了~</el-divider>
            </div>
            
            <!-- 空状态 -->
            <div v-if="collectedBlogList.length === 0 && !collectedLoading" class="empty-state">
              <el-empty description="还没有收藏的文章" />
            </div>
          </div>
        </el-tab-pane>
      </el-tabs>
...
import { User, Edit, Camera, Star, CollectionTag, Loading, View, StarFilled } from '@element-plus/icons-vue'
import { queryUserById, editUser, likedBlogs, collectBlogs } from '@/api/user'
...
// 喜欢的文章相关状态
const likedBlogList = ref([])
const likedLoading = ref(false)
const likedLoadingMore = ref(false)
const likedNoMoreData = ref(false)
const likedLastTime = ref('')
const likedContainerRef = ref()

// 收藏的文章相关状态
const collectedBlogList = ref([])
const collectedLoading = ref(false)
const collectedLoadingMore = ref(false)
const collectedNoMoreData = ref(false)
const collectedLastTime = ref('')
const collectedContainerRef = ref()
...
// 加载喜欢的博客
const loadLikedBlogs = async (isLoadMore = false) => {
  if (likedLoading.value || likedLoadingMore.value || (isLoadMore && likedNoMoreData.value)) {
    return
  }
  ...
  try {
    if (isLoadMore) {
      ...
      likedBlogList.value = []
      likedNoMoreData.value = false
    }

    const targetUserId = route.params.userId
    const addTime = isLoadMore ? likedLastTime.value : getCurrentTime()
    
    console.log('加载喜欢的文章:', { addTime, limit, targetUserId })
    
    const response = await likedBlogs(addTime, limit, targetUserId)
    
    if (response.code === 200 && response.data && response.data.length > 0) {
      if (isLoadMore) {
        likedBlogList.value.push(...response.data)
      } else {
        likedBlogList.value = response.data
      }
      
      // 更新最后一篇文章的时间
      if (response.data.length > 0) {
        likedLastTime.value = response.data[response.data.length - 1].articleAddTime
      }
      
      // 如果返回的数据少于请求的数量，说明没有更多数据了
      if (response.data.length < limit) {
        likedNoMoreData.value = true
      }
    } else {
      if (isLoadMore) {
        likedNoMoreData.value = true
...
// 加载收藏的博客
const loadCollectedBlogs = async (isLoadMore = false) => {
  if (collectedLoading.value || collectedLoadingMore.value || (isLoadMore && collectedNoMoreData.value)) {
    return
  }

  try {
    if (isLoadMore) {
      collectedLoadingMore.value = true
    } else {
      collectedLoading.value = true
      collectedBlogList.value = []
      collectedNoMoreData.value = false
    }

    const targetUserId = route.params.userId
    const addTime = isLoadMore ? collectedLastTime.value : getCurrentTime()
    
    console.log('加载收藏的文章:', { addTime, limit, targetUserId })
    
    const response = await collectBlogs(addTime, limit, targetUserId)
    
    if (response.code === 200 && response.data && response.data.length > 0) {
      if (isLoadMore) {
        collectedBlogList.value.push(...response.data)
      } else {
        collectedBlogList.value = response.data
      }
      
      // 更新最后一篇文章的时间
      if (response.data.length > 0) {
        collectedLastTime.value = response.data[response.data.length - 1].articleAddTime
      }
      
      // 如果返回的数据少于请求的数量，说明没有更多数据了
      if (response.data.length < limit) {
        collectedNoMoreData.value = true
      }
    } else {
      if (isLoadMore) {
        collectedNoMoreData.value = true
    ...
    collectedLoadingMore.value = false
  }
}
...
const handleTabClick = (tab) => {
  ...
  if (tab.name === 'liked') {
    // 如果喜欢列表为空，则加载
    if (likedBlogList.value.length === 0) {
      loadLikedBlogs()
    }
  } else if (tab.name === 'collected') {
    // 切换到收藏标签时，重置状态并加载收藏列表
    console.log('切换到收藏标签，开始加载收藏列表')
    collectedLastTime.value = ''
    collectedNoMoreData.value = false
    loadCollectedBlogs()
  }
}
