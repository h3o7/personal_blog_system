<template>
      ...
      <div class="article-section">
        <div class="section-header">
          ...
          <div class="search-info" v-if="searchKeyword">
            ...
            <span v-if="articles.length > 0">找到 {{ articles.length }} 篇相关文章</span>
          </div>
        </div>
        ...
        <div class="article-list" v-loading="loading && articles.length === 0" ref="articleListRef">
          <el-card v-for="article in articles" :key="`${article.userName}-${article.articleAddTime}`" class="article-card"
            ...
            <div class="article-title-section" @click="goToArticleDetail(article.articleId)">
              <h3 class="article-title" v-html="highlightKeyword(article.articleTitle)"></h3>
            </div>
  
            <!-- 文章内容 -->
            <div class="article-content" @click="goToArticleDetail(article.articleId)">
              <p class="article-text" v-html="highlightKeyword(article.articleContent)"></p>
            </div>
  ...
  import { View, Star, StarFilled, Loading, Search } from '@element-plus/icons-vue'
  import { ElMessage } from 'element-plus'
  import { searchArticle, likeArticle, collectArticle } from '@/api/article'
  ...
  // 获取搜索文章列表
  const loadSearchArticles = async (isLoadMore = false) => {
    const keyword = route.query.keyword
    
    if (!keyword) {
      ElMessage.error('搜索关键词不存在')
      return
    }
    ...
    try {
      ...
      console.log('搜索文章参数:', { keyword, limit: requestLimit })
      
      const response = await searchArticle(keyword, requestLimit)
  ...
  onMounted(() => {
    ...
    bannerArticles.value = [
      ...
      },
      {
        id: 2,
        title: '知识探索',
        summary: '在浩瀚的知识海洋中找到属于您的宝藏',
        coverImage: 'https://images.unsplash.com/photo-1481627834876-b7833e8f5570?ixlib=rb-4.0.3&auto=format&fit=crop&w=1200&q=80',
        publishTime: new Date().toISOString(),
        category: '知识分享'
      },
      {
        id: 3,
        title: '智能搜索',
        summary: '精准匹配，快速找到您需要的信息',
        coverImage: 'https://images.unsplash.com/photo-1558618666-fcd25c85cd64?ixlib=rb-4.0.3&auto=format&fit=crop&w=1200&q=80',
        publishTime: new Date().toISOString(),
        category: '搜索技术'
      }
    ]
