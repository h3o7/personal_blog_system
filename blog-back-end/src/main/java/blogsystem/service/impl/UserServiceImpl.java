...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements IUserService {
    ...
    public Result updateUser(User newUser) throws ClientException {
        String oldUserAvatar = userMapper.selectById(newUser.getUserId()).getUserAvatar();
        ...
        // 更新用户信息
        int flag = userMapper.updateById(newUser);
        if (flag > 0) {
            // 更新成功
    ...
    public Result getFollowingUsers(Integer pageNum, Integer pageSize) {
        ...
        for (UserFan userFan : userFans) {
            User user = userMapper.selectById(userFan.getUserId());
            if (user != null) {
                UserVO userVO = new UserVO();
                BeanUtils.copyProperties(user, userVO);
    ...
    public Result getFollowers(Integer pageNum, Integer pageSize) {
        ...
        List<UserVO> userVOs = new ArrayList<>();
        for (UserFan userFan : userFans) {
            User user = userMapper.selectById(userFan.getFanId());
            if (user != null) {
                UserVO userVO = new UserVO();
                BeanUtils.copyProperties(user, userVO);
//                userVO.setUserId();
